...
pt rotate(pt p, T angle){
    return {p.x * cos(angle) - p.y * sin(angle), p.x * sin(angle) + p.y * cos(angle)};
}

ld find_angle(pt a, pt b){
    T lo = 0,hi = 90 ;
    for(int i = 0;i<10000;i++){
        T mid = (lo + hi) / 2;
        if(cross(rotate(a, degree(mid)), b) > 0){
            lo = mid;
        }else{
            hi = mid;
        }
...
int main(){
    ...
    */
    ...
    // while(tc--){
        ...
        int can = inter_cl(c, l, out);
        // cout<<inter_cl(c, l, out)<<el;
        // cout<<out.ff<<el<<out.ss<<el;
        // cout<<rotate({r, 0}, degree(11.915197445846822))<<el;
        if(can){
            cout<<fixed<<setprecision(10)<<find_angle({r, 0}, out.ss)<<el;
        }else{
            cout<<-1<<el;
        }
    // }
